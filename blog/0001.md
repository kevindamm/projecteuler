...
const simple_count = ref(1)
const selection_count = ref(1)
...
const a = ref(3)
const b = ref(5)
const k = computed(() => constant_n/15)

function incrementSimple() { simple_count.value++ }
...
<div class="numbers-grid">
  <div class="numbers-plus" @click="incrementSimple"> +1 </div>
  <div class="numbers-cell" v-for="index in simple_count">{{ index }}</div>
</div>
...
The last row may be partial, a few values remaining between $15*k$ and $n$.
We can sum those up in a couple of small for loops, adding them to the main
result.  Or, since we know the row size consists of non-shared multiples of
$3$ and $5$, we can use some number theory to arrive at the result
without even having to loop through the numbers in the remainder:

```go
k := (n-1) / 15 // number of rows (except partial final row)
...
between $n$ and the last $k*15$ value.  You can see the sequence this creates
...
the extension to this which is summing over all multiples of an integer.

$$ \sum_i^n {a i} = a \sum_i^n i = a * \frac{n(n+1)}{2} $$
...
remainder search.  The LCM can then be derived from $(a*b)/$GCD$(a, b)$.
...
This site will not give it to you directly, but you can execute the code
examples here, with only minor edits, to find the answer.

The official overview stops at calculating the $(\sum 3) + (\sum 5) - (\sum 15)$
...
their response (a number or one of fizz, buzz, fizzbuzz).  I don't know the
